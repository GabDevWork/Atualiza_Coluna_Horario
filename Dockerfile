...
# Copiar arquivos do DBT e credenciais de conexão do Google
COPY ./<ARQUIVO_PROFILES>.yml ${DBT_PROFILES_DIR}/profiles.yml
COPY ./<ARQUIVO_CREDENCIAIS_SHEETS> /usr/datamart/
COPY ./ ./
...
RUN curl "https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip" -o "awscliv2.zip" \
    && unzip awscliv2.zip \
    && ./aws/install \
    && rm -rf awscliv2.zip ./aws # [cite: 2]

# Instalar pacotes do Python para ingestão de Sheets e persistência
